#-------------------------------------------------------- ห้ามขาย ให้เครดิตกูด้วย AnnmarieX หลุดเยอะเลยแจก --------------------------------------------------------
...
TOKEN = 'OTE4OTA2MzIyNzIyODgxNTk3.YbOEMw.3ZFUSWX0POq3tDV3E2f7k3USHoI' #token ของบอท
LIMIT = 32 #จำนวนสูงสุดที่ใส่ได้ต่อการยิง 1 ครั้ง
...
@bot.command()
async def help(ctx):
    await ctx.send(f"> **คำสั่งยิงเบอร์** | {PREFIX}sms [เบอร์] [จำนวนข้อความ 1 - {str(LIMIT)}ข้อความ]")

@bot.command()
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == '927045982636630026'):
        if (phone == None or amount == None):
            await ctx.send("> กรุณาใส่ข้อมูลให้ครบถ้วน ไม่งั้นเขาก็ไม่เอามึง")
            
        else:
            try:
                amount = int(amount)
                if (amount > LIMIT):
                    await ctx.send(f"> กรุณาใส่จำนวนข้อความไม่เกิน {str(LIMIT)} ข้อความยกเว้นกูจะเพิ่มให้นะสัส ")
                    

                else:
                    await ctx.send(f"> ลากหัวคมๆที่เบอร์ **{phone}** แล้ว | ยิงไป **{amount}**ข้อความ")
                    startall(phone, amount)
                    
            except:
                await ctx.send("> มึงใส่ไม่ถูกต้องไอ้สัส")

                
    else:
        await ctx.send("> ใช้ในห้องนี้ไอ้ควาย #ลากหัวเบอร์ครูคมๆ-บายลาบไอซ์")
keep_alive()
bot.run(TOKEN)
